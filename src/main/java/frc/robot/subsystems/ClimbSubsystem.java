...
public class ClimbSubsystem extends SubsystemBase {
    ...
    private final SparkMaxSim s_climbLeaderMotor = new SparkMaxSim(m_leaderMotor, motorSim);
    ...
    private final DutyCycleEncoder m_absoluteEncoder = new DutyCycleEncoder(ClimbConstants.kRevThroughBoreIO, 360, 0); // need
                                                                                                                       // to
                                                                                                                       // find
                                                                                                                       // exact
                                                                                                                       // angle
    ...
    public boolean atAngle() {
        return Math.abs(getClimbArmAngle() - ClimbConstants.kClimbForwardLimit) < ClimbConstants.kClimbTolerance;
    }
    ...
    public void periodic() {
        ...
        if (!(Robot.isSimulation())) {
            ...
            SmartDashboard.putNumber("Climb Motor Speed", m_leaderMotor.get());
            SmartDashboard.putNumber("Climb Motor (" + ClimbConstants.kClimbLeaderMotorCANID + ") Current",
                    m_leaderMotor.getOutputCurrent());
            SmartDashboard.putNumber("Climb Motor (" + ClimbConstants.kClimbFollowerMotorCANID + ") Current",
                    m_leaderMotor.getOutputCurrent());
    ...
    public void simulationPeriodic() {
        ...
        s_climbLeaderMotor.iterate(m_leaderMotor.getAppliedOutput(), 12, 0.02);
