...
public class ClimbSubsystem extends SubsystemBase {
    ...
    // variable used during initialization of robot to ensure initialization
    // functions stop running when they are no longer needed
    ...
    public ClimbSubsystem() {
        ...
        // IMPORTANT!
        m_climbLeaderMotorConfig.smartCurrentLimit(ClimbConstants.kMotorStallCurrent,
                ClimbConstants.kMotorFreeSpeedCurrent);
        m_climbFollowerMotorConfig.smartCurrentLimit(ClimbConstants.kMotorStallCurrent,
                ClimbConstants.kMotorFreeSpeedCurrent);

        // inverts the encoder on true (if motor is spinning counter-clockwise)
        m_absoluteEncoder.setInverted(false);
        // sets the duty cycle of the encoder to ensure accuracy (much match with
        // encoder specifications)
        m_absoluteEncoder.setDutyCycleRange(ClimbConstants.kRevThroughBoreMinPulse,
                ClimbConstants.kRevThroughBoreMaxPulse);
        // sets the expected frequency the encoder will output to the RIO (must match
        // with encoder specifications)
        // recommended to set this as it improves the reliability of the encoder's
        // output
        ...
        m_climbLeaderMotor.configure(m_climbLeaderMotorConfig, ResetMode.kResetSafeParameters,
                PersistMode.kPersistParameters);
        m_climbFollowerMotor.configure(m_climbFollowerMotorConfig, ResetMode.kResetSafeParameters,
                PersistMode.kPersistParameters);
    ...
    public void runToAngle(CommandXboxController m_operatorController, double currentAngle, double targetAngle,
            double direction) {
    ...
    // this is only called once during initialization of the robot and is run after
    // the climb is homed
    public void setClimbToAttack() {
        runToAngle(getClimbArmAngle(), ClimbConstants.kClimbForwardLimit, 1);
    }

    // drives the climb to its default position, which is 90 degrees from the
    // horizontal
    public void homeClimb() {
        if (setupClimb == 0) {
            if (getClimbArmAngle() == ClimbConstants.kClimbHomePose) { // to test, may require rounding
                ...
                setupClimb = 1;
            } else if (getClimbArmAngle() > ClimbConstants.kClimbHomePose) { // adjustment of encoder is necessary for
                                                                             // this to work
                ...
                // moves climb forward
                runToAngle(getClimbArmAngle(), ClimbConstants.kClimbHomePose, 1); // adjustment of encoder is necessary
                                                                                  // for this to work
            }
        }
    }
    ...
    public void periodic() {
        // determines if robot is being simulated. if so, will not display the metrics
        // below
        if (!(Robot.isSimulation())) {
            ...
            SmartDashboard.putBoolean("isFunnelUnlcooked", isFunnelUnlocked);
            // SmartDashboard.putNumber("Climb Motor Speed", m_climbLeaderMotor.get());
            // commented out for testing
            // SmartDashboard.putNumber("Climb Motor (" +
            // ClimbConstants.kClimbLeaderMotorCANID + ") Current",
            // m_climbLeaderMotor.getOutputCurrent());
            // SmartDashboard.putNumber("Climb Motor (" +
            // ClimbConstants.kClimbFollowerMotorCANID + ") Current",
            // m_climbLeaderMotor.getOutputCurrent());
    ...
    public void simulationPeriodic() {
        ...
        // SmartDashboard.putBoolean("Climb stopped",
        // s_climbLeaderMotor.getAppliedOutput() == 0);
        // SmartDashboard.putNumber("Motor Bus Voltage",
        // s_climbLeaderMotor.getBusVoltage());
        // SmartDashboard.putNumber("Motor Current",
        // s_climbLeaderMotor.getMotorCurrent());
        // SmartDashboard.putNumber("Climb Setpoint", s_climbLeaderMotor.getSetpoint());
        // SmartDashboard.putNumber("Applied Output",
        // s_climbLeaderMotor.getAppliedOutput());
